...
async function process(date) {
	...
	let Eod = await getPolygonEod(date)
	if (Eod !== null) { 
	...
	for (let ticker of universe.data) {
		...
		tickerData = await getTicker(ticker) // get past data data points
		if (tickerData !== null) { 
			console.log(`got data for ${ticker}`)
			tickerData.data.sort((a,b)=> {return(a.t-b.t)}) // sort by milliseconds
					// run filters
			result = await exp3daysupdown(tickerData.data)
			.then(console.log("complete 3daysupdown"))

			result = await exp2daysupdown(tickerData.data)
			.then(console.log("complete 2daysupdown"))

			result = await  gapupdown(tickerData.data)
			.then(console.log("complete gapupdown"))

			//update averages and other data points
			result = await computeDatapoints(tickerData.data, ticker, date)
