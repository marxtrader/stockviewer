...
async function getdata(ticker) {
	...
	let response = axios({
		method:"get",
		url:`http://localhost:4000/eod/${ticker}`,
...
async function getUniverse() {
	...
	let response = await axios({
		method:"get",
		url:`http://localhost:4000/eod/all?date=2021-03-26`,
		config:{ headers: {'Content-Type': 'application/json' }}
	})
...
async function averages(data, ticker) {
	//console.log(data)
	//console.log("**************************************")
...
async function process() {
	...
	for (let ticker of universe.data) {
		...
		tickerData = await getdata(ticker)
...
process().then(res => console.log("Done")).catch((err)=>console.log(err))
